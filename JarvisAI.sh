...
echo -e "\033[1;36m"
echo "================================================================"
echo "   J.A.R.V.I.S. AI • AUTONOMOUS NEURAL COMPUTER AGENT (Linux)  "
echo "================================================================"
echo -e "\033[0m"
...
if ! command -v node &> /dev/null; then
    echo -e "\033[1;31m[!] Node.js topilmadi. Node.js 18+ o'rnating: https://nodejs.org\033[0m"
    exit 1
fi
...
if curl -s -f "http://127.0.0.1:4141/ping" > /dev/null 2>&1; then
    AGENT_RUNNING=1
    echo -e "\033[1;32m[✓] Mahalliy kompyuter agenti faol (Port 4141).\033[0m"
else
    echo "[*] Mahalliy kompyuter agenti ishga tushirilmoqda (Port 4141)..."
    ...
    for i in {1..15}; do
        if curl -s -f "http://127.0.0.1:4141/ping" > /dev/null 2>&1; then
            echo -e "\033[1;32m[✓] Mahalliy kompyuter agenti tayyor (PID: $AGENT_PID).\033[0m"
            break
        fi
        sleep 0.2
    done
